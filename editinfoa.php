...
<body>
	...
	<form method="post" class="signin" action="editinfob.php">
		<font color="#fefcd7" style="font-family:bebas">EDIT OFFICIAL</font>
		<hr width="320" noshade color="#71B238"></hr>
		
		<input type="hidden" name="name1" value='<?php
		$con = mysql_connect("localhost","root","");
		if (!$con) {
		  die('Could not connect: ' . mysql_error());
		}
		...
		<input type="hidden" name="age1" value='<?php
		$con = mysql_connect("localhost","root","");
		if (!$con) {
		  die('Could not connect: ' . mysql_error());
		}
		...
		<input type="hidden" name="position1" value='<?php
		$con = mysql_connect("localhost","root","");
		if (!$con) {
		  die('Could not connect: ' . mysql_error());
		}
		mysql_select_db("dims", $con);
		$position1 = mysql_real_escape_string($_POST['position']);
		echo "".$position1."";
		?>'>
		
		<input type="hidden" name="otherinfo1" value='<?php
		$con = mysql_connect("localhost","root","");
		if (!$con) {
		  die('Could not connect: ' . mysql_error());
		}
		mysql_select_db("dims", $con);
		$otherinfo1 = mysql_real_escape_string($_POST['otherinfo1']);
		echo "".$otherinfo1."";
		?>'>
		
		<font type="text" size="2" color="#fefcd7" style="font-family:verdana; padding:5px">Name:</font>
		<br>
		<center><input name="name2" value='<?php
		$con = mysql_connect("localhost","root","");
		if (!$con) {
		  die('Could not connect: ' . mysql_error());
		}
		...
		<br>
		<center><input name="age2" value='<?php
		$con = mysql_connect("localhost","root","");
		if (!$con) {
		  die('Could not connect: ' . mysql_error());
		}
		...
		<br>
		<center><input name="position2" value='<?php
		$con = mysql_connect("localhost","root","");
		if (!$con) {
		  die('Could not connect: ' . mysql_error());
		}
		...
		<br>
		<center><input name="otherinfo2" value='<?php
		$con = mysql_connect("localhost","root","");
		if (!$con) {
		  die('Could not connect: ' . mysql_error());
		}
		mysql_select_db("dims", $con);
		$otherinfo1 = mysql_real_escape_string($_POST['otherinfo']);
		echo "".$otherinfo1."";
		?>' style="width:90%"></input></center>
		<hr width="320" noshade color="71B238"></hr>
